...
class Host:
    ...
    def udt_receive(self):
        ...
        if pkt_S is not None:
            ...
            if self.addr is 'H2':
                self.udt_send('H1', 'The way you look should be a sin, you my sinsation')
...
class Router:
    ...
    def send_routes(self, i): #ENCODING: NAME///NODE/LINK/COST//NODE/LINK/COST//....
        ...
        contents = self.name + "->"  # encode name of router
        for k, j in self.rt_tbl_D.items():
            for link, cost in j.items():
                contents += str(k) + "_" + str(link) + "_" + str(
                    cost) + "#"  # one '/' seperates route from link and cost, two '/' seperates route from other route
    ...
    def update_routes(self, p, i):
        updateFlag = 0
        contents = p.data_S.split("->")  # decode packet
        name = contents[0]
        routeList = contents[1].split("#")
        for i in routeList:  # for all routes in this routing table
            if i != '':
                nodeList = i.split("_")
